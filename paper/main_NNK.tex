...
% ---------- Title ----------
\title{CIGAR: A Combustion-Informed Physics-Based Model for Interpretable Prediction of Cigarette Mainstream Smoke Constituents}

\author{
Jizhao Guo$^{1}$, Yihang Zhu$^{2}$, Fan Wu$^{2}$,   Haifeng Li$^{3}$, \\
Yipeng Wang$^{1}$, Peijian Sun$^{1}$, Bin Peng$^{1}$, Cong Nie$^{1}$, \\
Xuehui Sun$^{1,*}$,  Qiang Zhang$^{2,*}$\\
\\
$^{1}$Zhengzhou Tobacco Research Institute of CNTC, 450001, China \\
$^{2}$Zhejiang University, 314400, China \\
$^{3}$China Tobacco Zhejiang Industrial Co., Ltd., 310008, China \\
$^{*}$Correspondence: xuehui\_sun1234@aliyun.com; \\
...
Combustible cigarettes remain the dominant form of tobacco use worldwide and a leading cause of preventable death, prompting sustained global regulatory efforts to monitor and reduce the harmful constituents of mainstream smoke. Predicting these constituents directly from a cigarette's design would accelerate product regulation and harm assessment, yet existing machine-learning approaches treat the cigarette as a black box, returning a single scalar yield with no account of the combustion process that produced it. We developed CIGAR (Combustion-Informed Generation And Retention), a physics-informed two-stage model that couples a mechanistic simulation of a cigarette's puff-by-puff combustion and filtration, driven by its physical design, with a learned formulation-specific generation factor predicted from tobacco chemical composition. Rather than emitting a single scalar yield, CIGAR produces a per-puff trajectory of generation at the combustion cone and retention through the tobacco rod and filter sections, with the mainstream yield as its endpoint. Under 5-fold cross-validation on a separate formulation-level dataset, CIGAR matches or outperforms strong data-driven baselines (XGBoost and TabPFN) on the majority of constituents---achieving the best mean absolute error on seven of nine and the best $R^2$ on six of nine---while delivering this simulated trajectory as a native output: a process-level account of how design choices propagate to mainstream emissions that black-box tabular models cannot reconstruct. Because every step of the trajectory is a physical state rather than a learned latent, the simulation is interpretable by construction and open to inspection at every puff and segment. By turning emission prediction into an inspectable simulation of the combustion process, CIGAR offers regulators and harm-reduction researchers a transparent tool for anticipating how design changes reshape the toxicant profile of mainstream smoke.
...
Figure~\ref{fig:mechanism_trajectory} visualizes the segment penetration
and puff-level CO trajectories produced by CIGAR for sample JH-7, a
Dataset~2 cigarette evaluated in one of the five cross-validation test
folds with CO as the target, while
Table~\ref{tab:mechanism_breakdown} reports the corresponding
intermediate values in full.
...
We
deliberately choose CO---one of the few constituents on which CIGAR does
not lead the data-driven baselines, with XGBoost the strongest model on
CO (Table~\ref{tab:model_comparison})---to show that the mechanistic
decomposition remains physically coherent even where CIGAR is not the
most accurate model. The
measured CO yield is $12.70\,\mathrm{mg}$; CIGAR predicts
$12.28\,\mathrm{mg}$ via the trajectory shown ($k_G = 0.8333$, absolute
error $0.42\,\mathrm{mg}$). As the cigarette burns and the tobacco rod
...
scalar endpoint; CIGAR additionally returns the full trajectory \emph{and} the
segment-level state that produced it.

\input{tables/mechanism_breakdown}
